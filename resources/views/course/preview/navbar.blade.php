<div class="fixed-action-btn horizontal" style="margin-bottom: 25px;">
  <a href="{{ url('/') }}/course/{{ $course_id }}/cirriculum" class="btn-floating btn-large red hoverable">
    ...
    @if ($lecture_ids[0] != $data->id)
      @foreach ($lecture_ids as $index=>$id)
        @if ($id == $data->id)
          ...
          <li>
            <a href="{{ url('/') }}/course/{{ $course_id }}/preview/{{ $lecture_ids[$index-1] }}" style="margin-top: -5px;" class="btn-floating green hoverable">
      ...
      <div class="collapsible-body" style="padding:0;">
        <ul class="collection blue">
          @foreach ($section->lecture as $lecture)
            <a href="{{ url('/') }}/course/{{ $course_id }}/preview/{{ $lecture->id }}">
